...
function! s:suite.test_new_bare() abort
...
function! s:suite.test_with_apply_exec() abort
...
function! s:suite.test_with_matcher_text() abort
...
function! s:suite.test_with_matcher_extension() abort
	call s:assert.equal(len(s:fixer.matchers), 0)
	let ext = 'go'
...
function! s:suite.test_visit_qfitem() abort
	open test.vim
	normal! iFirst Line
	normal! oSecond Line
	normal! oThird Line
...
function! s:suite.test_visit_qfitem_throws_on_invalid_qfitem() abort
...
function! s:suite.test_check_match() abort
...
function! s:suite.test_generate_matcher_key() abort
	let fixer = deepcopy(s:fixer)
	let prefix = 'foo'
